...
public class StudentDAO {
	...
	public void addStudent(StudentPO student){
		try {
			StudentPO unique = this.queryStudentByKey(student.getNo());
			if (unique!=null){
				throw new UniqueException("该学生已经注册了!");
			}
	...
	public ArrayList<StudentPO> queryStudents() {
		...
		try {
			if (cn != null) {
				...
				ResultSet rs = stmt.executeQuery("SELECT * FROM student order by Sno");
				while (rs.next()) {
					student = new StudentPO();
					student.setNo(rs.getString("Sno"));
					student.setName(rs.getString("Sname"));
